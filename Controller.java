...
public class Controller implements Initializable {
    ...
    private File logs;
    private OutputStreamWriter writeMsg;
    private InputStreamReader readLogs;
                ...
                try {
                    ...
                    while (true) {
                        ...
                        if (str.startsWith("/")) {
                            if (str.startsWith(Command.AUTH_OK)) {
                                ...
                                dateNow = new Date();
                                File dir = new File("E://Documents//Java//Geekbrains//chat_geekchat_16012021//logs");
                                logs = new File(dir, "chatlog_of_" + nickname + ".txt");
                                if (!logs.exists()) {
                                    logs.createNewFile();
                                }

                                ByteArrayOutputStream buf = new ByteArrayOutputStream();
                                readLogs = new InputStreamReader(new FileInputStream(logs), StandardCharsets.UTF_8);

                                    int result = readLogs.read();
                                    int countOfMsg = 0;
                                    while (result != -1 || countOfMsg == 100) {
                                        if ((char)result == '\n')
                                            countOfMsg++;
                                        buf.write((byte)result);
                                        result = readLogs.read();

                                    }
                                readLogs.close();

                                textArea.appendText(buf.toString());
                                break;
                            }
                    ...
                    while (true) {
                        ...
                        if (str.startsWith("/")) {
                            ...
                            FileOutputStream outMsg = new FileOutputStream(logs, true);
                            writeMsg = new OutputStreamWriter(outMsg, StandardCharsets.UTF_8);
                            try {

                                    String log = formatForDateNow.format(dateNow) + str + "\n";
                                    writeMsg.write(log);
                                    writeMsg.flush();
